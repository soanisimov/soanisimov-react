...
const SkillsProgress = () => {
    ...
    return (
        <section className="px-6 py-10" id="experience">
            ...
            <div className="max-w-7xl mx-auto bg-black/10 p-5 rounded-2xl filer backdrop-blur-[200px]">
                {SLILLS.map((skill, index) => (
                    <motion.div key={index} variants={progressLevel} initial="hidden"
                                className="text-xl md:text-2xl lg:text-3xl mb-6 leading-relaxed"
                                whileInView="visible"
                                viewport={{once: true, amount: 0.5}}>
                        <p className="text-sm md:text-2xl leading-relaxed mb-0.5">{skill.name}</p>
                        <div className="w-full rounded-full h-5">
                            <div className="w-full bg-sky-600/40 rounded-full h-4  mb-[-60px]">
                                <div className="rounded-full h-4 bg-blue-500" style={{width: `${skill.level}%`}}></div>
                            </div>
                        </div>
                    </motion.div>
                ))}
